...
app.post('/api/tasks', (req, res) => {
  try {
    ...
    const p = validPriorities.includes(priority) ? priority : 'P3';
    const stmt = db.prepare('INSERT INTO tasks (title, description, due_date, priority) VALUES (?, ?, ?, ?)');
    const result = stmt.run(title, description || '', due_date || null, p);
...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    const result = stmt.run(title, description || '', due_date || null, p, req.params.id);
